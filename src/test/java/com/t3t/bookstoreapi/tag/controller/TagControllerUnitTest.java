...
/**
 * {@link TagController} 클래스의 단위 테스트
 *
...
class TagControllerUnitTest {
    ...
    @Test
    void getTagList_thenReturns200() throws Exception {

        Pageable pageable = PageRequest.of(0, 10, Sort.by("tagId").ascending());
        PageResponse<TagDto> dummyResponse = PageResponse.<TagDto>builder()
                .content(Collections.singletonList(TagDto.builder().id(1L).build()))
                .pageNo(0)
                .pageSize(10)
                .totalElements(1)
                .totalPages(1)
                .last(true)
                .build();
        when(tagService.getTagList(pageable)).thenReturn(dummyResponse);
    ...
    @DisplayName("태그 목록 조회 테스트_데이터가 없는 경우 204 반환하는지 테스트")
    @Test
